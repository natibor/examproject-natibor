...
public class WorkSessionControllerIT {
    ...
    @Test
    void testUpdate_validIdAndCommand_successfulUpdate() throws Exception {
        createValidProject();
        createValidEmployee();
        createValidWorkSession();
    ...
    void testDelete_invalidId_notFoundResponse() throws Exception {
        mockMvc.perform(delete("/api/worksessions/1"))
    ...
    private void workSessionNotFound() throws Exception {
        mockMvc.perform(get("/api/worksessions/1"))
                .andExpect(status().isNotFound());
    }

    private void getEmptyList() throws Exception {
        mockMvc.perform(get("/api/worksessions"))
                .andExpect((status().isOk()))
